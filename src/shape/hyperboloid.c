...
int	is_inside_hyp(t_ray *ray, t_hyperboloid hyp)
{
	...
	h_point = vec_add(hyp.center, vec_scale(hyp.hgt, hyp.orient));
	cone_dist = vec_dot(vec_subs(ray->point_at, h_point), hyp.orient);
	cone_radius = (cone_dist / hyp.hgt) * hyp.radius;
	orth_distance = vec_len(vec_subs(vec_subs(ray->point_at, h_point),
				vec_scale(cone_dist, hyp.orient)));
	return (orth_distance <= cone_radius);
}

void	hyp_light_hit(t_ray *ray, t_data *data,
	t_hyperboloid hyp, t_light light)
{
	t_vec	dist;
	t_vec	proj;
	...
	if (!light_hit_objs(data, ray->point_at, light))
		return ;
	dist = vec_subs(ray->point_at, hyp.center);
	proj = vec_subs(dist, vec_scale(vec_dot(dist, hyp.orient), hyp.orient));
...
float	hit_hyperboloid(t_vec v, t_hyperboloid hyp, t_vec origin)
{
	...
	point_at1 = vec_add(origin,
			vec_scale((-b - sqrt(b * b - 4 * a * c)) / (2.0 * a), v));
	point_at2 = vec_add(origin,
			vec_scale((-b + sqrt(b * b - 4 * a * c)) / (2.0 * a), v));
	if (vec_dot(vec_subs(point_at1, hyp.h_point), vec_unit(hyp.h)) >= 0
		&& vec_dot(vec_subs(point_at1, hyp.h_point),
			vec_unit(hyp.h)) <= vec_len(hyp.h))
		return ((-b - sqrt(b * b - 4 * a * c)) / (2.0 * a));
	else if (vec_dot(vec_subs(point_at2, hyp.h_point), vec_unit(hyp.h)) >= 0
		&& vec_dot(vec_subs(point_at2, hyp.h_point),
			vec_unit(hyp.h)) <= vec_len(hyp.h))
		return ((-b + sqrt(b * b - 4 * a * c)) / (2.0 * a));
	return (-1);
}
...
float	it_hit_hy(t_data *data, t_ray *ray, t_hyperboloid hy)
{
	...
	ambient_color = add_color(vec_scale(K_LIGHT, hy.colors),
